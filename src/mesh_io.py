...
def load_ply(path_mesh):
    ...
    with open(path_mesh, 'r') as file:
        ...
        lines = file.readlines()[:n_header_max]

        # get number of lines in header
        mask = ['end_header' in line for line in lines]
        n_header = np.argmax(mask) + 1

        # get idx for number of verts
        mask = ['element vertex' in line for line in lines]
        idx_n_verts = np.argmax(mask)
        n_verts = int(lines[idx_n_verts].split(' ')[-1])

        # get idx for number of faces
        mask = ['element face' in line for line in lines]
        idx_n_faces = np.argmax(mask)
        n_faces = int(lines[idx_n_faces].split(' ')[-1])

    # load verts
    verts = np.loadtxt(path_mesh, skiprows=n_header, max_rows=n_verts)
    while np.isnan(verts[-1, -1]):
        verts = verts[:-1, :]

    # load faces
    faces = np.loadtxt(path_mesh, skiprows=n_header+n_verts, max_rows=n_faces, usecols=(1, 2, 3)).astype(int)
